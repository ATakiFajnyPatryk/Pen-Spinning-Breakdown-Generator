...
Kolejną rzeczą wartą zrobienia jest pozwolenie użytkownikowi przejrzeć listę wariantów trików, z których korzysta generator. Najlepiej będzie jeśli użytkownik będzie mógł je jakoś sensownie posegregować, na przykład po trudności albo przynależności do rodziny wariantów.
...
w końcu to projekt z baz danych. Najlepszym pomysłem jaki przyszedł mi do głowy, to umożliwienie użytkownikowi zapisania kombinacji trików, jeśli uzna, że jest ciekawa. Zapisywane kombinacje będą widoczne dla wszystkich użytkowników w jednej globalnej liście zapisanych kombinacji (w końcu to nie ma tu czegoś takiego jak konta użytkownika i dobrze). Zapisane kombinacje będą przechowywane w tabeli tricks_in_combo. Pojawia się tutaj kilka nowych pytań, na przykład o maksymalną ilość kombinacji pamiętanych w bazie, ale pozostawię te pytania bez odpowiedzi, bo jeszcze dokładnie tego nie przemyślałem. Poza tym nie wiem nawet, czy będę miał czas, żeby zrealizować tę dodatkową funkcjonalność.
