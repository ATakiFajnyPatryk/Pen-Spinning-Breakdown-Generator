...
W poleceniu na moodlu (tym dotyczącym drugiej części, czyli diagramu UML) jest napisane, żeby uzasadnić, że dane, które pojawiają się w projekcie są konieczne i wystarczają. To, że są konieczne już uzasadniłem dla każdego atrybuty podając jego zastosowanie. A to, że wystarczają, stanie się jasne poniżej, gdzie opisuję funkcjonalności. Należy jednak pamiętać, że są to dane wystarczające jedynie w uproszczonym modelu świata występującego w projekcie.
